...
use bitcoin::{
    block::Header,
    consensus::{self, encode, Decodable, ReadExt},
    hashes::Hash,
    io as bitcoin_io, secp256k1, Block, BlockHash, VarInt,
};
use bitcoin::{
    consensus::Encodable,
    p2p::{
        ...
        message_network, Address, Magic, ServiceFlags,
    },
    secp256k1::rand::Rng,
};
use thiserror::Error;
...
impl Decodable for RawNetworkMessage {
    fn consensus_decode<D: bitcoin_io::Read + ?Sized>(d: &mut D) -> Result<Self, encode::Error> {
...
impl Peer {
    pub async fn connect<A: ToSocketAddrs>(addr: A, magic: Magic) -> Result<Self, Error> {
        ...
        // Wait for handshake with timeout to avoid hanging if handler dies
        tokio::select! {
            _ = init_notifier.notified() => {},
            _ = tokio::time::sleep(tokio::time::Duration::from_secs(30)) => {
                if running_handler.is_finished() {
                    return Err(Error::PeerChannelClosed("handler died during handshake".into()));
                }
                return Err(Error::PeerChannelClosed("handshake timeout".into()));
            }
        }
    ...
    pub async fn get_new_headers(
        ...
        trace!("get_new_headers: {:?}", intersects);
    ...
    pub async fn get_blocks(&mut self, hashes: Vec<BlockHash>) -> Result<Vec<Block>, Error> {
        ...
        trace!("get_blocks finished downloading, decoding...");
        ...
        trace!("get_blocks finished decoding");
...
impl PeerHandler {
            ...
            maybe_msg = self.ingress_recv.recv() => {
                ...
                match msg {
                    ParsedNetworkMessage::Version(ver) => {
                        trace!("peer reported version {ver:?}, responding with ack");
    ...
    pub async fn run(&mut self) -> Result<(), Error> {
        debug!("sending initial version message to peer");
