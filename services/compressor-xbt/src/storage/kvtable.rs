use super::{options::ChainDBConfig, Error};
    ...
    V: From<Box<[u8]>>,
{
    ...
    fn cf(db: &'_ rocksdb::OptimisticTransactionDB) -> rocksdb::ColumnFamilyRef<'_> {
        db.cf_handle(Self::CF_NAME).unwrap()
    }

    fn reset(db: &rocksdb::OptimisticTransactionDB, config: ChainDBConfig) -> Result<(), Error> {
        db.drop_cf(Self::CF_NAME).map_err(Error::Rocks)?;

        let db_options = crate::storage::options::db_options(false, config);
