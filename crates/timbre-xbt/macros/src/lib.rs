        ...
        match input.data {
            Data::Struct(data) => {
                let fields = match data.fields {
                    ...
                    _ => return TokenStream::new(), // Only support named fields
                };
                ...
                quote! {
                    impl crate::encdec::Encode for #name {
                        fn encode(&self) -> Vec<u8> {
                            let mut encoder = crate::encdec::enc();
            ...
            _ => proc_macro2::TokenStream::new(), // Ignore other cases
        };
    ...
    let decode_impl = match input.data {
        Data::Struct(data) => {
            ...
            let field_decodes = fields.iter().map(|f| {
                ...
                quote! {
                    let (#field_name, rest) = <#field_ty as Decode>::decode(bytes)?;
                    bytes = rest;  // Update the slice to the remaining bytes after decoding
                }
            });

            quote! {
                impl crate::encdec::Decode for #name {
                    fn decode(bytes: &[u8]) -> Result<(Self, &[u8]), crate::TimbreError> {
                        ...
                        match &variant.fields {
                            ...
                            Fields::Unnamed(fields) => {
                                ...
                                let field_decodes = fields.unnamed.iter().enumerate().map(|(i, _)| {
                                    ...
                                    quote! {
                                        let (#field_name, bytes) = <#field_ty as Decode>::decode(bytes)?;
                                    }
                                });
                ...
                impl crate::encdec::Decode for #name {
                    fn decode(bytes: &[u8]) -> Result<(Self, &[u8]), crate::TimbreError> {
                        ...
                        match kind {
                            ...
                            #(#variant_decodings)*
                            _ => Err(TimbreError::MalformedInput("Unrecognized asset kind".into())),
                        }
                    }
                }
            }
        }
        _ => TokenStream::new().into(), // Only support struct data types
    };
